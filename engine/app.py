...
def run_task(task):
    ...
    container = docker_client.containers.create(
        ...
        command=cmd,
        tty=True,
        labels={'mgmt_flow.job_id': str(job_id), 'mgmt_flow.task_id': str(task_id)}
    )
    container.start()
    db_query('UPDATE flow_tasks SET container_id = %s WHERE id = %s', (container.id, task_id))

    result = container.wait()
    status_code = result.get('StatusCode', 1)
    log_bytes = container.logs(stdout=True, stderr=True, tail=2000)
    output = log_bytes.decode('utf-8', errors='replace')[:MAX_OUTPUT_BYTES]
    container.remove(force=True)

    status = 'complete' if status_code == 0 else 'failed'
    db_query(
        'UPDATE flow_tasks SET status = %s, exit_code = %s, output = %s, completed_at = now() WHERE id = %s',
        (status, status_code, output, task_id)
    )
